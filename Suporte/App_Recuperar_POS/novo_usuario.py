...
def mostrar_novo_usuario(supabase):
    ...
    # --- NOVO: Busca as empresas cadastradas no banco ---
    empresas_dict = {}
    try:
    ...
    # Cria a lista de opções para o Selectbox (vazio no início para forçar a escolha)
    opcoes_empresas = [""] + list(empresas_dict.keys())

    with st.form("form_novo_usuario", clear_on_submit=True):
        titulo_secao("Dados do colaborador")
        
        # Colunas reorganizadas para ficar 3x3 e manter o layout equilibrado
        ...
        if submit:
            # Valida se a empresa foi escolhida
            if not nome or not email or not senha or not empresa_nome:
                st.error("Preencha todos os campos obrigatórios.")
            else:
                nivel_num = int(nivel.split(" ")[0])
                empresa_id = empresas_dict[empresa_nome] # Resgata o ID correspondente ao nome selecionado
                    ...
                    try:
                        with st.spinner("Criando usuário..."):
                            ...
                            if resposta_auth.user:
                                ...
                                # Adiciona o empresa_id na hora de salvar o perfil
